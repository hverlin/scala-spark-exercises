...
class AirTrafficProcessorTest extends FlatSpec with GivenWhenThen with AppendedClues with TimeLimitedTests {
  ...
  val processor = new AirTrafficProcessor(sqlContext, flightPath, airportPath, carrierPath)

  val df = sqlContext.read.parquet(getClass().getResource("/sample.parquet/").toString().drop(5))
  ...
  "loadDataAndRegister" should "load the csv file register it as " +
    ...
    sqlContext.tableNames().contains("airtraffic") should be(true) withClue ("register table airtraffic")
  ...
  "timeSpentTaxiing" should "tell by airport how long airplanes taxi " +
    "by average" in {
    val res = Array(("PIT", 5.0), ("CVG", 6.166666666666667), ("ATL", 9.0), ("TPA", 9.25), ("LGA", 10.0), ("MCO", 11.0), ("BWI", 11.0), ("SNA", 14.5), ("SLC", 14.5), ("PHL", 17.0))
  ...
  "didNotFlyDataset" should "calculate which airliners didn't fly using datasets" in {
    processor.didNotFly(df).count() should equal(1490) withClue ("Count should be: 1490")
  }
}